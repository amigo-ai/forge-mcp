...
export function registerEntityTools(server: McpServer, pool: ClientPool): void {
  server.tool(
    ...
    async ({ entity_type, org_id }) => {
      ...
      const listKey = ENTITY_LIST_KEYS[entity_type as EntityType];
  ...
  server.tool(
    "forge_entity_create",
    "Create a new entity of the given type. Provide the entity data as a JSON object.",
    {
      entity_type: entityTypeSchema,
      data: z
  ...
  server.tool(
    ...
    "Update an existing entity by creating a new version. Provide the full version data.",
    {
      ...
      data: z
        .record(z.string(), z.unknown())
        .describe("The version data as a JSON object"),
      org_id: z.string().optional().describe("Org ID (uses active org if omitted)"),
    },
  ...
  server.tool(
    ...
    async ({ entity_type, entity_id, org_id }) => {
      ...
      await client.request(`${apiPath}/${entity_id}/`, {
        method: "DELETE",
      });
